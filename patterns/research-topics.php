<?php
/**
 * Title: ResearchTopics
 * Slug: theme_ai_center/researchTopics
 * Categories: featured, theme_ai_center/ai-center
 */
?>
<!-- wp:group {"layout":{"type":"constrained"}} -->
<div class="wp-block-group col-sm-12 col-md-8 col-3">
    <!-- wp:group {"style":{"layout":{"selfStretch":"fixed","flexSize":"25%"}},"layout":{"type":"constrained"}} -->
    <div class="wp-block-group sub-container-sm">
        <!-- wp:heading {"level":6,"className":"h6"} -->
        <h6 class="wp-block-heading h6">Research Topics</h6>
        <!-- /wp:heading -->
        
        <!-- wp:list {"className":"divider-list"} -->
        <ul class="divider-list">
            <!-- wp:list-item -->
            <li>Mathematical and Statistical Foundations of Machine Learning</li>
            <!-- /wp:list-item -->
            
            <!-- wp:list-item -->
            <li>Interpretability and Exunstyledability</li>
            <!-- /wp:list-item -->
            
            <!-- wp:list-item -->
            <li>Safety, Reliability, and Robustness</li>
            <!-- /wp:list-item -->
            
            <!-- wp:list-item -->
            <li>Fairness and Bias of AI Systems</li>
            <!-- /wp:list-item -->
            
            <!-- wp:list-item -->
            <li>Machine Perception</li>
            <!-- /wp:list-item -->
        </ul>
        <!-- /wp:list -->
    </div>
    <!-- /wp:group -->
</div>
<!-- /wp:group -->
